...
public class ComponentHelper {
    ...
    public static String toAnsiStringOrContent(IChatComponent component) {
        String string = toAnsiString(component);
        return string == null ? component.getUnformattedText() : string;
    }

    public static String toAnsiString(IChatComponent component) {
        ...
        if (AdventureWrapper.ENABLED) {
            ...
            try {
                if (!adventureHelperInitialized) {
                    adventureHelper = AdventureWrapper.getAdventureHelper(s -> true, (s, def) -> I18n.format(s));
